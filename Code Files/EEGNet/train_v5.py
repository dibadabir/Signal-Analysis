...
class EEGNetConfig:
    ...
    # ── Training ────────────────────────────────────────────
    batch_size:     int   = 32
    lr:             float = 1e-4   # IMPROVED: was 2e-4 — lower base LR, warmup handles the ramp
    weight_decay:   float = 5e-4   # IMPROVED: was 3e-4 — more L2 to penalise subject-specific memorisation
    max_epochs:     int   = 300    # IMPROVED: was 200
    min_epochs:     int   = 30     # IMPROVED: was 20
    patience:       int   = 40     # IMPROVED: was 35
    grad_clip_norm: float = 1.0

    # ── LR warmup ────────────────────────────────────────────
    # NEW: linear warmup over first warmup_epochs epochs.
    # LR ramps from near-zero to cfg.lr, preventing the overconfident
    # wrong predictions that caused folds 3 and 5 val loss to spike
    # upward in the first few epochs.
    ...
    # ── Label smoothing ──────────────────────────────────────
    label_smoothing:    float = 0.10   # IMPROVED: was 0.05 — higher smoothing softens overconfident early predictions
...
def train_one_epoch(
    ...
    for X_batch, y_batch in loader:
        ...
        if use_mixup:
            # Apply mixup BEFORE the forward pass
...
def _build_model_and_optimiser(
    ...
    # Start at lr/warmup_epochs; _run_training_loop linearly ramps to cfg.lr
    optimizer = torch.optim.Adam(
        model.parameters(),
        lr=cfg.lr / max(cfg.warmup_epochs, 1),
...
def _run_training_loop(
    ...
    for epoch in epoch_pbar:
        # ── Linear LR warmup ────────────────────────────────────────────
        # Ramp LR from cfg.lr/warmup_epochs up to cfg.lr over the first
        # warmup_epochs epochs, then hand off to ReduceLROnPlateau.
        # This prevents the overconfident wrong-prediction spike seen in
        # difficult folds (e.g. fold 3/5) at the very start of training.
        ...
        # Only let ReduceLROnPlateau act after warmup is complete
        if epoch > cfg.warmup_epochs:
...
def run_cross_validation(
    ...
    fold_preds:        List[Dict]        = []   # NEW: raw per-fold predictions
    cv_t0 = time.time()
    ...
    return results_df, fold_train_losses, fold_val_losses, fold_preds
...
def select_best_fold_model(
    ...
    """
    Select a CV fold as the final model, then load its checkpoint.

    If `fold` is specified, that fold is used directly (manual override).
    Otherwise the fold with the highest `metric` value is chosen automatically.

    No additional training is performed — the chosen fold's weights are used
    directly as the final model.
    """
    _banner("Step 3 / 4 — Selecting fold as final model")

    if fold is not None:
        # Manual override
        if fold not in results_df["fold"].values:
            raise ValueError(
                f"Fold {fold} not found in CV results. "
                f"Available folds: {sorted(results_df['fold'].tolist())}"
            )
        best_fold = fold
        best_val  = float(results_df.loc[results_df["fold"] == fold, metric].iloc[0])
        print(f"  Selection mode   : manual override")
        print(f"  Selected fold    : Fold {best_fold}  ({metric} = {best_val:.4f})")
    else:
        # Automatic — pick by metric
        best_row  = results_df.loc[results_df[metric].idxmax()]
        best_fold = int(best_row["fold"])
        best_val  = float(best_row[metric])
        print(f"  Selection mode   : automatic  (metric = {metric})")
        print(f"  Selected fold    : Fold {best_fold}  ({metric} = {best_val:.4f})")

    print()
    print(f"  {'Fold':<6} {'subj_auc':>9} {'subj_acc':>9} {'subj_bal_acc':>13}  {'selected':>8}")
    print(f"  {'─'*6} {'─'*9} {'─'*9} {'─'*13}  {'─'*8}")
    for _, row in results_df.iterrows():
        marker = "  ◀ selected" if int(row["fold"]) == best_fold else ""
        print(
            f"  {int(row['fold']):<6} {row['subj_auc']:>9.4f} "
            f"{row['subj_acc']:>9.4f} {row['subj_bal_acc']:>13.4f}{marker}"
        )

    # Load checkpoint
    ckpt_src  = MODEL_DIR / f"best_fold{best_fold}.pt"
    ckpt_dest = MODEL_DIR / "final_model.pt"
    if not ckpt_src.exists():
        raise FileNotFoundError(
            f"Checkpoint for best fold not found: {ckpt_src}\n"
            "Ensure cross-validation completed successfully."
        )

    model, _, _ = _build_model_and_optimiser(cfg)
    model.load_state_dict(torch.load(ckpt_src, map_location=cfg.device))
    model.eval()

    # Copy to final_model.pt so downstream code has a consistent path
    import shutil
    shutil.copy2(ckpt_src, ckpt_dest)
    print(f"\n  Checkpoint copied : {ckpt_src.name}  →  {ckpt_dest.name}")
    ...
    print(f"\n  ✅ Fold {best_fold} model ready  ({metric}={best_val:.4f})")
...
def evaluate_held_out_subjects(
    ...
    return results, subj_y, subj_probs


# ============================================================
# Results saving — called at end of main()
# ============================================================

def _comparison_label_names(comparison: str) -> Dict[int, str]:
    return {
        "OFF_CTL": {1: "PD-OFF", 0: "CTL"},
        "ON_CTL":  {1: "PD-ON",  0: "CTL"},
        "ON_OFF":  {1: "PD-ON",  0: "PD-OFF"},
    }.get(comparison, {1: "Class 1", 0: "Class 0"})


def save_cv_metrics_summary(results_df: pd.DataFrame, save_dir: Path) -> Path:
    """
    Write cv_metrics_summary.tsv — mean ± SD / min / max of every CV metric.
    """
    metric_cols = [
        "ep_acc", "ep_auc", "ep_bal_acc",
        "subj_acc", "subj_auc", "subj_bal_acc",
    ]
    rows = []
    for col in metric_cols:
        ...
        rows.append({
            ...
            "mean_pm_sd": f"{s.mean():.4f} ± {s.std():.4f}",
        })
    out = save_dir / "cv_metrics_summary.tsv"
    pd.DataFrame(rows).to_csv(out, sep="\t", index=False)
    print(f"  CV metrics summary   -> {out}")
    return out
...
def plot_roc_curves_cv(
    ...
    """
    roc_curves_cv.png — per-fold ROC lines + shaded ±1 SD band around the mean.
    Uses subject-level probabilities (one probability per held-out subject per fold).
    """
    ...
    palette = sns.color_palette("muted", n_colors=len(fold_preds))

    for fp in fold_preds:
        sy, sp = fp["subj_y"], fp["subj_probs"]
        ...
        ax.plot(
            common_fpr, tprs[-1],
            lw=1.2, alpha=0.55,
            color=palette[fold_num - 1],
            label=f"Fold {fold_num}  (AUC={fold_auc:.3f})",
        )
    ...
    if tprs:
        ...
        ax.plot(common_fpr, mean_tpr, "k-", lw=2.2,
    ...
    ax.plot([0, 1], [0, 1], "r--", lw=1, alpha=0.6, label="Chance")
    ax.set_xlim([-0.01, 1.01])
    ax.set_ylim([-0.01, 1.05])
    ax.set_xlabel("False Positive Rate", fontsize=11)
    ax.set_ylabel("True Positive Rate",  fontsize=11)
    ax.set_title(f"ROC — CV folds (subject-level)\n{comparison}", fontsize=12)
    ax.legend(fontsize=8, loc="lower right")
    fig.tight_layout()
    out = save_dir / "roc_curves_cv.png"
    fig.savefig(out, dpi=150)
    plt.close(fig)
    print(f"  ROC curves (CV)      -> {out}")
    return out


def plot_roc_curve_held_out(
    subj_y:     np.ndarray,
    subj_probs: np.ndarray,
    save_dir:   Path,
    comparison: str = "OFF_CTL",
) -> Path:
    """
    roc_curve_held_out.png — single ROC curve for the held-out test subjects.
    """
    from sklearn.metrics import roc_curve, auc as sk_auc
    ...
    ax.set_xlabel("False Positive Rate", fontsize=11)
    ax.set_ylabel("True Positive Rate",  fontsize=11)
    ax.set_title(f"ROC — Held-out subjects\n{comparison}", fontsize=12)
    ax.legend(fontsize=10, loc="lower right")
    fig.tight_layout()
    out = save_dir / "roc_curve_held_out.png"
    fig.savefig(out, dpi=150)
    plt.close(fig)
    print(f"  ROC curve (held-out) -> {out}")
    return out


def plot_confusion_matrix_cv_aggregated(
    fold_preds: List[Dict],
    save_dir:   Path,
    comparison: str = "OFF_CTL",
) -> Path:
    """
    confusion_matrix_cv_aggregated.png — CM built from all CV fold predictions
    concatenated together (subject-level, one prediction per held-out subject).
    """
    label_names = _comparison_label_names(comparison)
    ...
    out = save_dir / "confusion_matrix_cv_aggregated.png"
    plot_confusion_matrix(
        np.array(all_y), np.array(all_pred),
        save_dir=save_dir,
        title=f"Aggregated CV confusion matrix\n({comparison}, subject-level)",
        filename="confusion_matrix_cv_aggregated.png",
        labels=labels,
    )
    print(f"  Aggregated CV CM     -> {out}")
    return out


def save_fold_classification_reports(
    fold_preds:  List[Dict],
    results_df:  pd.DataFrame,
    save_dir:    Path,
    comparison:  str = "OFF_CTL",
    ...
    """
    Save fold{n}_classification_report.txt for each CV fold.
    Each file contains epoch-level and subject-level sklearn reports.
    """
    label_names  = _comparison_label_names(comparison)
    ...
    for fp in fold_preds:
        ...
        # Epoch-level
        ep_y    = fp["ep_y"]
        ep_pred = (fp["ep_probs"] >= 0.5).astype(int)
        ep_report = classification_report(
            ep_y, ep_pred, target_names=target_names, zero_division=0
        )

        # Subject-level
        sy      = fp["subj_y"]
        sp      = fp["subj_probs"]
        s_pred  = (sp >= 0.5).astype(int)
        subj_report = classification_report(
            sy, s_pred, target_names=target_names, zero_division=0
        )

        # Row from results_df
        row = results_df[results_df["fold"] == fold].iloc[0]

        lines = [
            f"Fold {fold} Classification Report",
            "=" * 50,
            f"Best epoch     : {int(row['best_epoch'])}",
            f"Val epochs     : {int(row['n_val_epochs'])}",
            f"Val subjects   : {int(row['n_val_subjs'])}",
            "",
            "── Epoch-level ─────────────────────────────────",
            ep_report,
            "── Subject-level ───────────────────────────────",
        ...
        out = save_dir / f"fold{fold}_classification_report.txt"
        out.write_text("\n".join(lines))
        saved.append(out)

    print(f"  Fold reports         -> fold1…fold{len(saved)}_classification_report.txt")
    return saved
...
def save_training_summary(
    ...
    """
    training_summary.json — single machine-readable record with every metric,
    config value, augmentation setting, wall time, and list of saved files.
    """
    metric_cols = [
        "ep_acc", "ep_auc", "ep_bal_acc",
        "subj_acc", "subj_auc", "subj_bal_acc",
    ...
    per_fold = results_df.to_dict(orient="records")

    # Saved files
    saved_files = [
        str(save_dir / "training_summary.json"),
        str(save_dir / "cv_metrics_summary.tsv"),
        str(save_dir / "roc_curves_cv.png"),
        str(save_dir / "roc_curve_held_out.png"),
        str(save_dir / "confusion_matrix_cv_aggregated.png"),
        str(save_dir / "cv_results.tsv"),
        str(save_dir / "held_out_results.tsv"),
        str(save_dir / "confusion_matrix_held_out.png"),
        str(save_dir / "confusion_matrix_train.png"),
        str(save_dir / "cv_summary.png"),
        str(save_dir / "learning_curves.png"),
        str(save_dir / "final_model.pt"),
        str(save_dir / "config.json"),
        str(save_dir / "norm_mean.npy"),
        str(save_dir / "norm_std.npy"),
    ]
    for fi in range(1, cfg.n_folds + 1):
        saved_files.append(str(save_dir / f"fold{fi}_classification_report.txt"))
        saved_files.append(str(save_dir / f"best_fold{fi}.pt"))

    summary = {
        "wall_time_s":    round(wall_time_s, 2),
        "wall_time_hms":  _fmt_time(wall_time_s),
        "config":         {
    ...
    out = save_dir / "training_summary.json"
    with open(out, "w") as fh:
        json.dump(summary, fh, indent=2)
    print(f"  Training summary     -> {out}")
    return out
...
def main() -> None:
    ...
    print("\n  Splitting held-out test subjects (5 PD-OFF + 5 CTL)...")
    X_tr, y_tr, g_tr, X_te, y_te, g_te, held_out_ids = split_holdout_subjects(
        X, y, groups, n_per_class=5
    )
    ...
    results_df, fold_train_losses, fold_val_losses, fold_preds = run_cross_validation(
        X_tr, y_tr, g_tr, CFG
    )
    ...
    # ── 3. Select fold as final model ───────────────────────────────────
    final_model = select_best_fold_model(X_tr, y_tr, CFG, results_df, metric="subj_auc", fold=4)
    ...
    held_out_results, held_out_subj_y, held_out_subj_probs = evaluate_held_out_subjects(
        model=final_model,
        X_te=X_te,
        y_te=y_te,
        g_te=g_te,
    ...
    # ── 5. Save all results ──────────────────────────────────────────────
    _banner("Step 5 / 5 — Saving results")
    save_cv_metrics_summary(results_df, MODEL_DIR)
    plot_roc_curves_cv(fold_preds, MODEL_DIR, comparison=CFG.comparison)
    plot_roc_curve_held_out(held_out_subj_y, held_out_subj_probs, MODEL_DIR, CFG.comparison)
    plot_confusion_matrix_cv_aggregated(fold_preds, MODEL_DIR, CFG.comparison)
    save_fold_classification_reports(fold_preds, results_df, MODEL_DIR, CFG.comparison)
