...
# Epoch timing
# CHANGED: full ±2s epoch so extract_condition_epochs() can crop to
# -250..1250ms. The old values (-0.2, 0.8) were too short for that.
TMIN     = -2.0   # seconds
TMAX     =  2.0   # seconds
BASELINE = None   # per-condition baseline done in extract_condition_epochs()
...
def select_subject_files(
    ...
    """
    CHANGED: returns (filepath, med_label) pairs where med_label is
    'ON', 'OFF', or 'CTL'.  This drives the output filename so each
    session gets its own .fif file: sub-XXX_ON-epo.fif, etc.
    """
    ...
    for f in files:
        ...
        if idx == 1:
            ...
            lf = f.lower()
            if any(t in lf for t in ["ses-1", "ses-01", "s1", "sess1"]):
                sess_key = "sess1"
            elif any(t in lf for t in ["ses-2", "ses-02", "s2", "sess2"]):
                sess_key = "sess2"
            else:
                sess_key = "sess1"
    ...
    # CTL fallback: if no session entity found, use first file with CTL label
...
for subj in tqdm(subjects, desc="Preprocess binary PD/CTL subjects"):
    ...
    # CHANGED: returns (filepath, med_label) pairs — one per session
    ...
    # Each (filepath, med_label) pair is processed and saved as its own .fif file.
    # e.g. sub-804_ON-epo.fif, sub-804_OFF-epo.fif, sub-890_CTL-epo.fif
    for f, med_label in files_and_labels:
        ...
        try:
            ...
            if n_eeg_full < 8:
                logger.warning(
                    f"sub-{subj} [{med_label}]: too few EEG channels ({n_eeg_full}); skipping."
                )
                continue
            ...
            if n_eeg < 8:
                logger.warning(
                    f"sub-{subj} [{med_label}]: too few EEG channels after selection "
                    f"({n_eeg}); skipping."
                )
                continue
            ...
            if events.size > 0 and event_id:
                ...
                if len(epochs) == 0:
                    logger.warning(
                        f"sub-{subj} [{med_label}]: all event epochs rejected "
                        f"(threshold={REJECT_PTP_EVENT*1e6:.0f} uV). "
                ...
                logger.info(
                    f"sub-{subj} [{med_label}]: no event markers found "
            ...
            if len(epochs) == 0:
                logger.warning(
                    f"sub-{subj} [{med_label}]: 0 epochs after fallback; skipping."
                )
                continue
            ...
            logger.warning(
                f"sub-{subj} [{med_label}]: preprocessing failed for "
                f"{Path(f).name} -> {e}"
            )

        # ── Save this session's epochs ─────────────────────────────────────
        if not epochs_list:
            logger.warning(f"sub-{subj} [{med_label}]: no usable epochs; skipping session.")
            subject_summary.append({
                "subject": subj, "group": selection_map[subj]["group"],
        ...
        # CHANGED: filename encodes medication status (ON / OFF / CTL)
...
# CHANGED: glob matches new ON/OFF/CTL filename pattern
